...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if(!auth||!db||!tools){message("Firebase is not configured yet. Check the project configuration.","error");submit.disabled=true;return;}
  auth.onAuthStateChanged(async user=>{try{if(user&&await authorized(user))location.replace("dashboard.html");}catch(error){console.warn("Existing administrator session could not be verified:",error);}});
  form.addEventListener("submit",async e=>{e.preventDefault();message("");if(!email.value.trim()||!password.value){message("Enter your email and password.","error");return;}loading(true);try{const result=await auth.signInWithEmailAndPassword(email.value.trim(),password.value);if(!(await authorized(result.user))){await tools.signOut(auth);message("This account does not have administrator access.","error");return;}message("Access verified. Opening the dashboard…","success");location.replace("dashboard.html");}catch(err){console.error(err);message(friendly(err),"error");}finally{loading(false);}});
});
